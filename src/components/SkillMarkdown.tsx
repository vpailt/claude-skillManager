    ...
    <div className={cn("prose-skill", className)}>
      {frontmatter && (
        <details className="mb-4 rounded-md border border-dashed border-border bg-muted/30">
          <summary className="cursor-pointer select-none px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
            Frontmatter
          </summary>
          <pre className="m-0 max-w-full overflow-x-auto rounded-none border-0 border-t border-border bg-transparent px-3 py-2 text-[11px] leading-relaxed text-foreground/80">
            <code className="bg-transparent p-0 text-[11px]">{frontmatter}</code>
          </pre>
        </details>
      )}
      <ReactMarkdown
